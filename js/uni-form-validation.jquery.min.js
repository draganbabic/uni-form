...
return true;},validateSameAs:function(field,caption){var classes=field.attr('class').split(' '),target_field='',target_field_name='',target_field_caption='';for(var i=0;i<classes.length;i++){if(classes[i]==='validateSameAs'){if(classes[i+1]!='undefined'){target_field_name=classes[i+1];break;}}}
if(target_field_name){target_field=jQuery('input[name="'+target_field_name+'"]');if(target_field.length>0){if(target_field.val()!=field.val()){target_field_caption=get_label_text(target_field);return i18n('same_as',caption,target_field_caption);}}}
...
return i18n('date',caption);},validateCallback:function(field,caption){var classes=field.attr('class').split(' '),callback_function='';for(var i=0;i<classes.length;i++){if(classes[i]==='validateCallback'){if(classes[i+1]!='undefined'){callback_function=classes[i+1];break;}}}
...
return text.replace('*','').replace(':','');};var i18n=function(lang_key){var lang_string=i18n_strings[lang_key],bits=lang_string.split('%'),out=bits[0],re=/^([ds])(.*)$/,p;for(var i=1;i<bits.length;i++){p=re.exec(bits[i]);if(!p||arguments[i]===null){continue;}
if(p[1]==='d'){out+=parseInt(arguments[i],10);}else if(p[1]==='s'){out+=arguments[i];}
out+=p[2];}
return out;};var showFormError=function(form,title,messages){var m,$message;if($('#errorMsg').length){$('#errorMsg').remove();}
...
form.prepend($message);$('html, body').animate({scrollTop:form.offset().top},500);$('#errorMsg').slideDown();return false;};var showFormSuccess=function(form,title){var $message;if($('#okMsg').length){$('#okMsg').remove();}
