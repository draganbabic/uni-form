...
form.submit(function(){var return_val;form.removeClass('failedSubmit');settings.ask_on_leave=false;form.removeClass('askOnLeave');form.find(settings.field_selector).each(function(){if($(this).val()===$(this).data('default-value')){$(this).val('');}});if(settings.prevent_submit||form.hasClass('preventSubmit')){form.find(settings.field_selector).each(function(){$(this).blur();});if(form.find('.'+settings.invalid_class).add('.'+settings.error_class).length){return_val=($.isFunction(settings.prevent_submit_callback))?settings.prevent_submit_callback(form,i18n('submit_msg'),[i18n('submit_help')]):showFormError(form,i18n('submit_msg'),[i18n('submit_help')]);}
if($.isFunction(settings.submit_callback)&&!settings.submit_callback(form)){return_val=false;}}
